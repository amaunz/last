      ...
      if ( legs[i]->tuple.depth != 0 ) {
        if ( ( totalsymmetry || legs[i]->tuple.depth <= edgelabels.size () / 2 ) &&
          ...
          if ( ( !fm::do_pruning || 
            ...
            if (max.first<fm::chisq->p) { fm::updated = true; max = tree.expand ( pair<float, string>(fm::chisq->p, fm::graphstate->to_s(legs[i]->occurrences.frequency))); }
            else max = tree.expand (max);
          }
