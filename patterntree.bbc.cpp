...
pair<float, string> PatternTree::expand (pair<float, string> max) {
  ...
  for ( int i = legs.size () - 1; i >= 0; i-- ) {
    ...
    if ( ( !fm::do_pruning || 
        ...
        // return most specialized pattern
        if (fm::most_specific_trees_only && fm::do_output && !fm::do_backbone && p.legs.size() == 0) {
            if (!fm::console_out) (*fm::result) << fm::graphstate->to_s(legs[i]->occurrences.frequency);
            else fm::graphstate->print(legs[i]->occurrences.frequency);
        }

        if (fm::chisq->p > max.first) { max = p.expand (pair<float, string>(fm::chisq->p,fm::graphstate->to_s(legs[i]->occurrences.frequency))); }
        else max = p.expand (max);
    }
    else {
            return max.second;
    }
